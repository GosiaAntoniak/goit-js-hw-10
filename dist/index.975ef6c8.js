...
searchBox.addEventListener("input", (0, _lodashDebounceDefault.default)(inputListener, DEBOUNCE_DELAY));
function inputListener(event) {
    countryList.innerHTML = null;
    countryInfo.innerHTML = null;
    const searchValue = event.target.value;
    const isSearchValueInvalid = /[^A-Za-z ]/.test(searchValue);
    if (isSearchValueInvalid) (0, _notiflix.Notify).failure("Please enter only letters. Do not use any numbers or special characters.");
    if (searchValue.length > 0 && !isSearchValueInvalid) fetchAndRenderCountries(searchValue);
}
...
function fetchAndRenderCountries(searchValue) {
    (0, _fetchCountriesDefault.default)(searchValue).then((searchResult)=>{
...
function fetchCountries(name) {
    return fetch(`https://restcountries.com/v2/name/${name.trim()}?fields=name,capital,population,flags,languages`).then((response)=>{
